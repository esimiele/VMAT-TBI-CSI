...
namespace VMATTBICSIAutoPlanningHelpers.Helpers
{
    ...
    {
        public static StringBuilder PrintPlanTemplateConfigurationParameters(List<CSIAutoPlanTemplate> templates) 
        { 
            ...
            foreach (CSIAutoPlanTemplate itr in templates.Where(x => !string.Equals(x.GetTemplateName(), "--select--")))
            {
                ...
                if (itr.GetRequestedOptTSStructures().Any())
                {
                    ...
                    {
                        sb.AppendLine(String.Format(" {0, -15} | {1, -9:N1} | {2,-10:N1} | {3,-5:N1} | {4,-8} |", ts.Item1, ts.Item2, ts.Item3, ts.Item4, ts.Item5));
